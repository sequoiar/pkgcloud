 ...
 * Client.getProject
 *
 * @description helper function for get a project via remote API.
 *
 * @param project_id
 * @param callback
 * @returns {*}
 * @private
 */
Client.prototype.getProject = function (project_id, callback) {
  var project_id = (typeof project_id === 'object') ? 
  project_id.project_id : project_id;
  this.emit('log::trace', 'Lookup project', property);
...
Client.prototype.getTenant = function(project_id, callback) {
  return this.getProject(project_id, callback);
};
 ...
 * Client.createRole
 *
 * @description helper function for create a role via remote API.
 *
 * @param property
 * @param callback
 * @returns {*}
 * @private
 */
Client.prototype.createRole = function (property, callback) {
  this.emit('log::trace', 'Creating role', property);
  ...
  return this._request({
    ...
    return err
      ? callback(err)
      : callback(null, body.role);
  });
};

/**
 * Client.getRoleByName
 *
 * @description helper function for get a role by name via remote API.
 *
 * @param name
 * @param callback
 * @returns {*}
 * @private
 */
Client.prototype.getRoleByName = function (name, callback) {
  this.emit('log::trace', 'Get role', name);

  return this._request({
    method: 'GET',
    path: '/v3/roles'+'?name='+name,
  }, function (err, body) {
    return err
      ? callback(err)
      : callback(null, body.roles && body.roles[0]);
  });
};

/**
 * Client.deleteRoleById
 *
 * @description helper function for delete a role by id via remote API.
 *
 * @param id
 * @param callback
 * @returns {*}
 * @private
 */
Client.prototype.deleteRoleById = function (id, callback) {
  this.emit('log::trace', 'Deleting role', id);
 ...
 *
 * @description helper function for grant role for user via remote API.
 *
 * @param property
 * @param property.project_id project id
 * @param property.user_id    user id
 * @param property.role_id    role id
 * @param callback
 * @returns {*}
 * @private
 */
Client.prototype.grantUserRole = function (property, callback) {
  this.emit('log::trace', 'Grant role ', property);
  var project_id = property.project_id;
  var user_id = property.user_id;
  var role_id = property.role_id;
